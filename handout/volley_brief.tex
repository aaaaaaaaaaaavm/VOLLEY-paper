...
\textcolor{ox}{a self-contained electromagnetic deployer. 77 programme run-sheet files covering 72 analysis identifiers, four independent solvers.
...
\textcolor{ox}{\textbf{The discipline}} & \textcolor{ox}{\textbf{77 programme run-sheet files covering 72 analysis identifiers. Each declares its acceptance bands, and commits them, \emph{before} the script that produces the number exists} --- provable by checking that the band commit touched no script. A failed band moves the design point. \textbf{No band has ever been widened after a result was known.}}\\
\end{tabular}\par}
...
\textbf{4} & \textbf{Every kilogram was attributed to the requirement causing it, and then every requirement was deleted.} Across all \textbf{64 corners}, \textbf{88.67\,kg survives} --- 70.06\,\% of dry mass, \textbf{7.39\,kg per satellite against a 2\,kg criterion.} \emph{The evaluated deletion combinations miss the mass target, and it is the single result that redirected the work.}\\
\end{tabular}\par}
...
\midrule
$\Delta v$ per satellite & 1--2.5 m/s, preset spring energy & \textcolor{ox}{16.03 m/s, commanded} & $\sim$16 m/s\\
Designed differential & no commanded profile & \textcolor{ox}{\textbf{16.03 m/s, to 0.027 (3$\sigma$)}} & per satellite\\
...
the computed ratio is 1.758. So is the constellation-phasing claim --- 30$^\circ$ of in-track phase
...
semi-major axis} a commanded shot does, beyond the reference 2.5 m/s spring case. No cost comparison is offered in either direction; there is no
...
{\scriptsize The next evidence step is a benchtop track and a mass simulator ---
...
The defect register carries \textbf{153 numbered entries, 53 of them still open}, including every
