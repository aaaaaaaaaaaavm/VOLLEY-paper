...
\textcolor{ox}{a self-contained electromagnetic deployer. 64 run-sheet files covering 61 analyses, four independent solvers.
...
\textcolor{ox}{\textbf{The discipline}} & \textcolor{ox}{\textbf{64 run-sheet files covering 61 analyses. Each declares its acceptance bands, and commits them, \emph{before} the script that produces the number exists} --- provable by checking that the band commit touched no script. A failed band moves the design point. \textbf{No band has ever been widened after a result was known.}}\\
\end{tabular}\par}
