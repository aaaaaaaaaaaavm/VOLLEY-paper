...
ejects it adds 1--2\,m/s --- enough to drift clear of the stage, not enough to change an orbit ---
...
\vspace{1.6mm}
\begin{center}
\includegraphics[width=0.90\linewidth,trim=125 195 25 35,clip]{img/hero.png}
\end{center}
\vspace{-1.0mm}
...
claimed here is \textbf{a commanded per-satellite velocity, on a satellite that is never modified,
at an acceleration inside its existing qualification envelope, from a stage that was going to be
discarded.} Each of those alone is ordinary. Together they describe a service tier with no
demonstrated occupant.\par}
...
Orbital energy & negligible & \textcolor{ox}{+28.8 km, $\times$1.60 life} & comparable\\
...
machine is not needed for it. \textbf{What survives is the third row: only $\Delta v$ changes an
orbit, and a clock cannot.} No cost comparison is offered in either direction; there is no
...
The defect register carries \textbf{132 numbered entries, 53 of them still open}, including every
