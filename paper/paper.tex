...
Electrical$\rightarrow$payload efficiency & 14.9--19.9\% \cite{feng2025} & 18.8\% (Sec.~V)\\
...
\begin{equation}
K_t = 10.54~\mathrm{N/(kA/m)},\quad \text{ripple } \pm1.01\%\ (6\text{th harm.}),
\end{equation}
shown in Fig.~\ref{fig:ripple}. Fig.~\ref{fig:fieldmap} shows the field the integral is taken over, and the reason the integral must be taken over it: sampled on the array's centre plane $B_y$ peaks at \textbf{0.5041~T}, while the mean across the 90~mm depth is \textbf{0.4759~T}, a ratio of \textbf{0.9440}. Treating the centre-plane value as though it held across the depth is the 4.42\% overstatement Sec.~\ref{sec:lim} records.

\begin{figure*}[t]
\centering\includegraphics[width=0.86\textwidth]{A02_field_map.png}
\caption{Halbach airgap field. Left, $B_y$ in the $x$--$y$ plane over one wavelength; right, the profile through the array's 90~mm depth. Drawn by the same field builder the thrust integral uses, so the two cannot disagree.}
\label{fig:fieldmap}
\end{figure*} Synchronous extraction carries the one-half traveling-wave factor that lumped $\langle B\rangle KA$ models hide; the corrected constant is recovered operationally by rating the sheet current at 140~kA/m and commanding 126~kA/m (21~A/mm$^2$ pulse-only in the copper), where adiabatic heating remains 0.3~K per 162~ms shot.
...
At the rated point (3U reference, 9.445~kg sled computed from the CAD solid volumes, 1.3~m acceleration): exit velocity \textbf{16.0~m/s} at \textbf{10.1~g}, pulse 162~ms, peak current 320~A, and bank state-of-charge sag 5.2\% (Fig.~\ref{fig:shot}). Energy drawn is 2.78~kJ per shot, with copper heat of 835~J and a further 83~J dissipated in the bank's own series resistance. Regenerative braking returns 291~J after release, so the net draw is 2.74~kJ. The payload receives 514~J, a \textbf{18.8\% electrical-to-payload efficiency}; the sled's share is split between 291~J recovered and 935~J dissipated in the arrest brake. Efficiency falls with the heavier sled for two compounding reasons: a larger fraction of the same mechanical work goes into a mass that is then braked away, and the longer 162~ms pulse accrues more copper loss at unchanged current density. Recovery is what partially offsets the first of those, and it is bounded by geometry rather than by loss: over the available 39~mm the braking pulse lasts 15.8~ms and burns 15~J of copper, against 835~J over the 162~ms shot, so recovered energy rises monotonically with braking force up to the winding's rating and with distance beyond it. A 500~mm section would return 570~J and a 1~m section would arrest the sled outright while returning 783~J, but the closed envelope already exceeds its target class by 44\% (Sec.~\ref{sec:lim}), so distance is not available here. Peak current during recovery is 219~A, below the shot's own 320~A, so the drive is not re-rated. The closed-loop Monte Carlo returns a dispersion of \textbf{0.0274~m/s (3$\sigma$)} at a 15.8~m/s fleet setpoint (Fig.~\ref{fig:mc}), dominated by the sensing chain rather than plant tolerances. The setpoint must sit below the open-loop ceiling for the servo to retain authority; at 98.85\% of it, the nominal headroom is 0.188~m/s. Table~\ref{tab:family} gives the payload family: the system is force-limited above 1U, so heavier payloads trade velocity, never structural margin, and the deployer mass each customer pays for varies by a factor of 109 across the family (Sec.~\ref{sec:family}).
...
\begin{table}[t]
\caption{Payload family at the rated point (1.3 m acceleration, 9.445 kg CAD-derived sled). Deployer mass per satellite uses the 126.6 kg dry mass and a packing model calibrated so the 3U case returns the twelve the magazine is laid out for. Every column is regenerated from \texttt{payload\_family.json} at the current operating point; the values published until 2026-08-20 predated both ADR-030 and the enclosure buildup}
\label{tab:family}
...
\midrule
PocketQube 1P (0.25 kg) & 18.9 m/s & 14.0 g & 326 & \textbf{0.388}\\
...
1U (1.33 kg) & 17.9 m/s & 12.6 g & 40$^{\dagger}$ & 3.165$^{\dagger}$\\
3U (4 kg) & 16.0 m/s & 10.1 g & 12 & \textbf{10.550}\\
...
Table~\ref{tab:family} is read most usefully from its last column rather than its first. Exit velocity is nearly flat across the family, because the 9.445~kg sled is most of the moving mass: removing the payload entirely buys 19\% of velocity, so velocity is a property of the sled and the stroke and not of the customer. Shortening the magnet array does not recover it either, since array length sets thrust and sled mass together---a 150~mm array gives a 5.15~kg sled but a 4.65~N per kA/m thrust constant, and the 3U case falls to 12.9~m/s.

The column that does move is deployer mass per satellite: by a factor of 27 between the 3U case and PocketQube~1P, and by 109 across the full column. At 3U the machine spends \textbf{10.55~kg of deployer per customer}, which loses to a cold-gas propulsion module delivering the same 16.0~m/s at 0.5--1.2~kg by a factor of between nine and twenty-one---12.4 against the 0.85~kg mid-class figure. At PocketQube~1P it spends \textbf{0.388~kg} and wins by 1.3 to 3.1. The comparison inverts on payload class, and on no machine parameter at all: not the thrust constant, not the stroke, not the sheet current. It inverts less decisively than it did: the figures published until 2026-08-20 were 7.04 and 0.24~kg, and the enclosure buildup that replaced an 8.00~kg packaging placeholder with 50.04~kg of derived line items moved both.
...
The comparison the 3U column loses most clearly is against the incumbent it was meant to match. A canisterised spring dispenser occupies roughly 6~kg per 3U slot, and an acceptance band written before the comparison script existed asked for parity within 15\%. The measured ratio is \textbf{1.758}: this deployer is 76\% heavier per satellite than the hardware it would displace, and the band fails. It failed on a change to this work rather than to the comparator---a packaging rollup that had carried an 8.00~kg placeholder was replaced by 50.04~kg of derived line items, and parity did not survive it. The project's own kill criterion on mass per satellite, fixed at an estimated 2~kg, is crossed by \textbf{5.3$\times$}. No enclosure change reaches parity: the lightest alternative structure costed, a honeycomb sandwich, would give 8.87~kg per satellite and a ratio of 1.48. The route that moves this criterion is a smaller payload class, which is what the left of Table~\ref{tab:family} prices and what Sec.~\ref{sec:lim} records has not been designed.

It is not for want of looking. Every kilogram of the dry mass was attributed to the requirement that causes it---six requirements, from \emph{the satellite is unmodified} to \emph{twelve satellites share one drive}---and each of the 64 subsets was then deleted in turn to find the lightest machine any combination of relaxations could reach (Fig.~\ref{fig:ledger}). Three of the six carry no mass on their own. The best corner deletes both of the two that do, giving up the reusable mover and the requirement that the energy arrive during the shot, and it removes \textbf{37.89~kg, or 29.90\%}. \textbf{88.67~kg survives every deletion in all 64 corners}---70.06\% of dry mass, \textbf{7.39~kg per satellite against a 2~kg criterion}. Deleting requirements does not close it; only carrying more, smaller satellites does. The ledger is an attribution and an upper bound on what relaxation buys, not a sizing model, and it does not claim the surviving mass is irreducible by better design.

\begin{figure*}[t]
\centering\includegraphics[width=0.92\textwidth]{A35_ledger.png}
\caption{Constraint ledger: dry mass attributed to the requirement causing it, and the floor reached by deleting requirements in all 64 combinations. The floor is 88.67~kg, 70.06\% of the 126.56~kg rollup.}
\label{fig:ledger}
\end{figure*}

Two competitive arguments were made for this machine and one of them is now withdrawn. Mass parity with a canister of springs is the withdrawn one. In-track phase spacing was withdrawn earlier and for a different reason---a clock delivers it, as Sec.~\ref{sec:astro} records. What is left is narrower and is the claim this paper defends: a commanded, per-satellite change in orbital energy, on a satellite that is never modified, at an acceleration inside its existing qualification envelope. It is bought at 1.758 times a dispenser's mass per satellite, and that price is stated here rather than left for a reader to compute from the table.

\subsection{Astrodynamic Utility}\label{sec:astro}
A prograde ejection raises apogee while leaving perigee at the deployment altitude, so the correct claim is a longer-lived ellipse. At 450~km with ballistic coefficient 61~kg/m$^2$, one 16.0~m/s shot multiplies lifetime by \textbf{1.60} at mean solar activity (base lifetimes 0.52--2.61~yr across the activity range; Figs.~\ref{fig:life} and \ref{fig:uq}).
...
Differential ejection velocities put adjacent satellites on different periods: splits of 2, 5, and 10~m/s reach 30$^\circ$ spacing in \textbf{6.9, 2.8, and 1.4 days}. Under identical density and geometry assumptions, differential drag between 3:1 area configurations achieves the same spacing in \textbf{25.0 days} (Fig.~\ref{fig:dragvs}), consistent with flown drag-phasing campaigns operating over weeks to months \cite{foster}.

Neither of those is the baseline in-track phase should be priced against, and an earlier version of this work omitted the one that is. Satellites released at different times from the same host arrive at different true anomalies in the same orbit at no velocity cost, and the in-track rate at 450~km is 0.0641$^\circ$/s: 30$^\circ$ of phase costs \textbf{468~seconds of waiting}, 255 times faster than the fastest commanded split above and free. The 1200~s inter-shot cadence adopted for thermal and recharge reasons already yields 76.9$^\circ$ per shot before the motor is energised. \textbf{The claim that a spring cannot reach 30$^\circ$ of phase by design is false}---a spring and a clock reach it---and it is withdrawn here rather than defended.

The free method is also the better one for this purpose. A commanded velocity split sets a phase \emph{rate}, not an offset: at 10~m/s the separation passes through 30$^\circ$ at 1.4 days and continues at 21.75$^\circ$/day indefinitely, and a satellite with no propulsion cannot null a drift it has been given. Timed release sets an offset in the same orbit at zero relative rate, and it holds.

What a clock cannot do is change an orbit, and that is where the separation between the two lies. After a timed release the semi-major axis equals the host's to within \textbf{0~m} and the orbital lifetime multiplier is \textbf{1.0000}; after a commanded shot the semi-major axis differs by \textbf{28.8~km} and the multiplier is \textbf{1.60}. Raised apogee, a chosen altitude shell, and +60.2\% of orbital life against a spring's +8.2\% are unreachable by waiting, by drag, or by a spring at any preload. \textbf{The differentiator is orbit change, not phase change}, and the superlinearity of lifetime in $\Delta v$---+6.5\% at 2~m/s, +8.2\% at 2.5, +60.2\% at 16.0---argues for more velocity rather than less. Impulsive seeding followed by drag trim remains available as a hybrid, but it is a refinement of the orbit-change claim and not of the phase one.
...
\begin{table}[t]
\caption{Per-shot recoil (64.1 N$\cdot$s) versus host class}
\label{tab:recoil}
\centering\footnotesize
\begin{tabular}{lcc}
\toprule
...
\begin{table}[t]
\caption{Service tiers for small-satellite orbit placement. VOLLEY's system mass is 126.6~kg dry and 174.6~kg loaded with twelve 3U satellites}
\label{tab:tier}
...
System mass, 12$\times$3U & 10--20 kg & 126.6 kg dry & 100s kg + prop.\\
...
\subsection{What Air Costs a Ground Test}\label{sec:airdrag}
The machine operates in vacuum and every velocity above is computed with no aerodynamic term, but the full-scale demonstration fires a mass simulator down the track in a laboratory at sea level, so the comparison that test exists to make is between a measured velocity and a computed one that omits the air. A steady RANS solution ($k$-$\omega$ SST) around the sled and payload \emph{as generated}, meshed directly from the CAD surfaces rather than from an idealised body, gives a drag coefficient of \textbf{0.52} on a 201.6~cm$^2$ projected frontal area at $Re = 7.0\times10^5$, i.e. 1.73~N at exit velocity. Because the profile is position-scheduled, $v^2$ rises linearly with distance and so does the drag, making the work over the 1.30~m acceleration zone 1.13~J and the exit-velocity deficit \textbf{5.1~mm/s}, or 0.031~\% (Fig.~\ref{fig:airdrag}).

That figure is small against the design point and \emph{not} small against the quantity a ground test resolves: it is \textbf{19~\% of the 3$\sigma$ closed-loop dispersion}. A full-scale test therefore needs no vacuum chamber, but every velocity it reports must carry the air correction, or it is compared to the model with an error one-fifth the size of the dispersion being verified. The distinction depends on how the test is run: open-loop, the deficit appears as 5.1~mm/s of velocity; closed-loop, the servo nulls it and it appears instead as 1.73~N of additional commanded force, 0.125~\% of the shot.

\begin{figure}[t]
\centering\includegraphics[width=\columnwidth]{A29_wake.png}
\caption{Mid-plane slice ($z=0$) of the converged fine-mesh solution: kinematic pressure above, speed below, flow left to right. Stagnation on the forward face, separation at the shoulders, and a wake that has not recovered by $x=2.2$~m. The pressure term shown here is integrated directly from the solved field; \textbf{the viscous term is not solved} and is bounded by a turbulent flat-plate correlation over the meshed wetted area, as the text states.}
\label{fig:wake}
\end{figure}

\begin{figure*}[t]
\centering\includegraphics[width=0.98\textwidth]{A29_cfd_report.png}
\caption{Force history and mesh convergence for the same case. The force is a mean over a window of iterations quoted with its spread, $1.734 \pm 0.144$~N, because a steady solver on a massively separated wake plateaus rather than converges; $6.3\times$ the cells moves the answer by 4.86~\%.}
\label{fig:cfdconv}
\end{figure*}

Two properties of the solution are reported rather than presented as convergence. The velocity residuals fall three orders and then plateau near $5\times10^{-2}$, which is what a steady solver does on a massively separated wake, so the force is a mean over a window of iterations quoted with its spread, $1.734 \pm 0.144$~N. And the viscous contribution is a turbulent flat-plate bound over the meshed wetted area rather than a solved wall shear stress, 17.7~\% of the total; the pressure term, which dominates, is integrated directly from the solved field over the body surface. Adding the stator plates \emph{reduces} drag by 13~\% and halves the oscillation, the plate acting as a wake splitter rather than as a confining wall, so the free-stream figure quoted above is the conservative one.

\begin{figure}[t]
\centering\includegraphics[width=\columnwidth]{F14_airdrag.png}
\caption{Air drag along the acceleration zone, and the resulting exit-velocity deficit set against the design point and against the dispersion a ground test exists to resolve.}
\label{fig:airdrag}
\end{figure}
